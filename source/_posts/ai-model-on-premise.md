...
> 注：host.docker.internal是Docker提供的一个特殊DNS名称，指向宿主机(Host)的内部IP地址。当容器需要访问宿主机上运行的服务(如数据库、API或其他应用)时，可以使用这个地址。除此之外，也可以使用Host网络模式，在容器启动时添加`--network host`参数，此时使用localhost(127.0.0.1)可以直接访问宿主机服务。使用Host网络模式后，容器将不会拥有自己独立的IP地址，任何`-p`端口映射参数都会被忽略并产生警告，容器中所有监听的端口都会抢占宿主机上的同名端口，需要注意端口冲突。
